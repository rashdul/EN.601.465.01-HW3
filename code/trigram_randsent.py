...
def sample(lm: LanguageModel, nt: int) -> str:
    """"
    Generate tokens based on language model.
    Args:
        lm: Language model to use
        nt: number of tokens to generate
    Return:
    ...
    while True:
        probs = []
        for z in lm.vocab:
            probs.append(2 ** lm.log_prob(x,y,z))
            # print(f'log prob of {z}: {2**lm.log_prob(x,y,z)}')
        # break
        sample = torch.multinomial(torch.tensor(probs), 1,replacement= True)
        sampled = lm.vocab[sample.item()]

        if sampled == "EOS" or i >= nt:
            break
        x, y = y, sampled
        generated_sen += sampled + " "
        i += 1

    return generated_sen.strip()
...
def main():
    ...
    for _ in range(args.Num_of_sentences):
        print(sample(lm, args.max_length))
